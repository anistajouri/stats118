...
export class DepartementComponent implements OnInit {
  ...
  docs : Array<any>;
  dep_clicked: number;
  name_dep_clicked: string;
  dephovered:  string;
  progdata: number;
  dep_sup_postion: number;
  closeResult: string;
  items:any;
  type_rech: string;
  type_int: number;

  constructor(private modalService: NgbModal,public dialog: MatDialog,private localityService: LocalityService, private proximityService: ProximityService) { 
 
  }

  setTypelocalite(){
    this.type_rech =  "localité";
    this.type_int = 1;
  }
  setTypeproximite(){
    this.type_rech =  "proximité";
    this.type_int = 2;
  }

  setDep_clicked(dep,i,content){
    var d = parseInt(dep);
    this.dep_clicked =d;
    this.name_dep_clicked = this.departements[i].title;
    //this.modalService.open(content, {backdropClass: 'light-blue-backdrop' });
  
    //this.loadlocalityfordep(dep);
    this.  openDialog();
  }
  ...
  openDialog() {
    const dialogRef = this.dialog.open(Dialogmap, {
      ...
      data:{"name_dep_clicked":this.name_dep_clicked,
            "dep_clicked": this.dep_clicked,
            "percent":this.progdata,
            "type_rech": this.type_rech,
  ...
  openVerticallyCentered(content) {
    var opt = { centered: true };
    this.modalService.open(content, { windowClass: 'modal-dialog-centered' });
  }
  ...
  ngOnInit() {
    this.type_rech =  "localité";
    this.type_int = 1;
    console.log("range"+ this.bsValue);
   ...
   this.onValueChange(this.bsValue);
    ...
    activedep($event,id){
      ...
      for( var i = 0; i< this.departements.length; i++){
        if($event.type == 'mouseover' && this.departements[i].id == id)
        {this.departements[i].class="active";
        this.progdata =  this.departements[i].percent;
        this.dephovered= this.departements[i].title;
      }
        else this.departements[i].class ="";
  ...
  onValueChange(value: Date): void {
    var local= value.toLocaleString();
    if(local.indexOf(',') > 0)
    {
      var range = this.formateRangeDate(value);
      this.date_d = range[0];
      this.date_f = range[1];
      console.log("llllll "+ this.date_d);
    ...
    var df_str = this.bsValue.toLocaleString();
    var d = this.formatDate(df_str);
    this.date_d = d;
      this.date_f = d;
    this.loadPercentagePerDepartment_df(d,d);
  ...
  refresh()
  {
      ...
      for(var key in this.percentages)
      {
        ...
        for(var i = 0; i < this.departements.length; i++){
          if(parseInt(key) == parseInt(this.departements[i].id))
            this.departements[i].percent = this.percentages[key];
    ...
    let max = 0;
      for(var i = 0; i < this.departements.length; i++){
      {
        if(this.departements[i].percent > max ){
        max = this.departements[i].percent;
        this.dep_sup_postion = i;
      }
      
      if(this.departements[i].percent >= 3 ){
        this.departements[i].fill ="#376b25" ;
      }
      else  if((this.departements[i].percent >= 1) && (this.departements[i].percent < 3) ){
        this.departements[i].fill ="#55a53a" ;
      }
      else this.departements[i].fill ="#92d17c" ;
      
    }
    
    }
    
  console.log("sipppp "+this.dep_sup_postion)
        this.departements[this.dep_sup_postion].class = "active";
      
        this.dep_clicked = parseInt(this.departements[this.dep_sup_postion].id);
        this.name_dep_clicked = this.departements[this.dep_sup_postion].title
        this.dephovered= this.name_dep_clicked;
  ...
  loadPercentagePerDepartment_df(date_debut, date_fin) {
    ...
    this.proximityService.getPercentagePerDepartment_df(date_debut, date_fin).subscribe(data => {
  ...
  loadlocalityfordep(dep)
  {
    this.localityService.getlocalityfordep_df(this.date_d, this.date_f, dep).subscribe(data => {
